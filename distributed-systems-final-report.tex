...
\paragraph{HUB Nodes (3+ instances)}
...
\paragraph{Load Balancer (1 nginx instance)}
...
\paragraph{OPC UA Servers (N instances)}
...
\paragraph{HTTP Clients (N instances)}
...
\begin{itemize}
  \item \textbf{TokenResponse (JWT)}: contiene JWT firmato più metadati (tipo token "bearer", ruolo utente, expiration 24h),
  permettendo ai client di autenticare richieste successive;
  \item \textbf{DataPointResponse}: singolo campionamento industriale con tag, valore, timestamp, quality status,
  Lamport clock, e identificativi server.
  \item \textbf{HistoryResponse}: contiene array di data point per range temporale richiesto più
  metadati (count, node\_id servente).
\end{itemize}
...
\begin{itemize}
    \item Latest value: \texttt{SELECT * WHERE tag=? AND server\_name=? ORDER BY lc DESC LIMIT 1}
    \item History: \texttt{SELECT * WHERE tag=? AND timestamp BETWEEN ? AND ? ORDER BY lc ASC}
\end{itemize}

\paragraph{Write Queries (Serialized)}
Batch insert every 5 seconds (OPC UA) + sync (Anti-Entropy) and SQLite serializes write.

\paragraph{Conflict Handling}
...
The system implements eventual consistency~\cite{vogels2009eventually}: given enough time and
...
\begin{itemize}
    \item Interval: 5s;
    \item Timeout: 3s each ping;
    \item Threshold: 3 missed heartbeats (15s) - peer marked as dead.
\end{itemize}

\paragraph{Properties}
\begin{itemize}
    \item \textbf{Strong Completeness}: every failed node detected by all survivors
    (if the network is potentially reliable)
...
\begin{description}
    \item[OPC UA Connection Failure] Retry exponential backoff (0.5s - 30s, max 20 attempts).
    Node remains operational (serves client with cached data). Alert via log.
    
    \item[Load Balancer Health Check Failure] nginx removes nodes from the pool after 1 failed check. Automatic retry, re-add when it returns to healthy status.
\end{description}
...
nginx round-robin on available nodes:
\begin{itemize}
    \item Health check: GET \texttt{/api/v1/health} every 30 seconds;
    \item Failure: automatic removal after 1 failed check;
    \item Recovery: automatic re-add after 1 successful check.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Eventual-Consistency Temporarily Lost}: Node C does not receive data from A/B and vice versa.
    Clients see divergent states;
...
% \hline
% \textbf{Threat} & \textbf{Attack} & \textbf{Mitigation} \\
% \hline
% Unauthorized access & Client senza JWT & JWT verification \\
% Privilege escalation & User - admin API & RBAC verify\_admin \\
% MITM & Network sniffing & TLS 1.3 \\
% Token theft & JWT leaked & Short expiration (24h) + HTTPS \\
% Brute force & Password guessing & Rate limiting (nginx) \\
% SQL injection & Malicious params & SQLAlchemy ORM \\
% DoS & Flood requests & nginx rate + connection limits \\
% \hline
% \end{tabular}
% \caption{Threat model e mitigazioni}
% \label{tab:threat-model}
% \end{table}

\paragraph{Known Limitations}
No token revocation (stateless JWT), no password hashing, no audit logging, no per-user
rate limiting. Acceptable for academic prototypes.
...
All HTTP messages carry JSON payloads. JSON was chosen over XML/Protobuf/MessagePack
...
The JWT implementation uses the python-jose library, chosen for being lightweight (no heavy
dependencies), supporting HS256/RS256/ES256 algorithms, actively maintained, and consistent with FastAPI
ecosystem examples. Token generation encodes payload with subject, role, scope, expiration (24h), and
issued-at claim, signing with shared secret JWT\_SECRET using HS256. Token verification decodes
with secret, validating signature and expiration, raising HTTPException 401 for invalid tokens.
JWT\_SECRET using HS256. Token verification decodes with secret, validating signature
and expiration, raising HTTPException 401 for invalid tokens.

RBAC enforcement uses FastAPI Dependency Injection pattern: dependency \texttt{verify\_admin()}
checks role equals “admin”, raising HTTPException 403 otherwise. Admin endpoints
declare this dependency, making them unreachable for non-admin tokens.
...
\textbf{Docker} containerizza nodi HUB (base python:3.11-slim), nginx (nginx:alpine),
e simulatori OPC UA (immagine custom server asyncua). \textbf{Docker Compose}
orchestra deployment multi-container con bridge network per comunicazione interna, volume
per storage persistente SQLite, direttive healthcheck per gestione dipendenze, e injection
variabili ambiente per secret e configurazione.
...
\begin{enumerate}
    ...
    \item Query database node-a: \texttt{SELECT COUNT(*) FROM data\_points WHERE server\_name='OPCServer1'};
...
\begin{enumerate}
    ...
    \item Manual startup of opc-server-4;
...
curl -X POST https://localhost/api/v1/admin/opc-servers \
  ...
  -d '{"server_name":"OPCServer4","endpoint":"opc.tcp://opc-server-4:4843"}'
    ...
    \item Query database on node-b and node-c: \texttt{SELECT * FROM server\_configs WHERE server\_name='OPCServer4'};
...
\paragraph{Test Procedure}
\begin{enumerate}
    \item Identify a recent data point on node-a: \texttt{SELECT * FROM data\_points ORDER BY lamport\_clock DESC LIMIT 1};
    \item Note the LC of the data point (es. LC=5234)
    \item Query node-b: \texttt{SELECT * FROM data\_points WHERE lamport\_clock=5234};
...
\begin{enumerate}
    \item Operating system with 3 nodes, all in sync (verified: max LC equal);
    \item Create a network partition using iptables:
    \begin{verbatim}
# blocking communication node-c - node-a/node-b
docker exec node-c iptables -A INPUT -s node-a -j DROP
docker exec node-c iptables -A INPUT -s node-b -j DROP
    \end{verbatim}
    ...
    \begin{verbatim}
docker exec node-c iptables -F  # flush rules
    \end{verbatim}
...
\textbf{Durante Partizione (T0 a T0+60s)}:
\begin{itemize}
...
[node-a] Anti-Entropy completato: 95 data points synchronized
    \end{verbatim}
    \item T0+85s: Full convergence verified:
    \begin{itemize}
        \item node-a: max LC = 6420;
        \item node-b: max LC = 6420;
        \item node-c: max LC = 6420.
    \end{itemize}
\end{itemize}

Convergence time: 25 seconds. Lamport clock guarantees
correct ordering: data points with higher LC preserved in case of conflicts (no
conflicts observed in this test because different source\_servers).
...
\paragraph{Actual Results}
Deployment successful. All containers started successfully:

% TODO mettere un verbatim reale
\begin{verbatim}
NAME          STATUS                    PORTS
node-a        Up 65s (healthy)          8000/tcp
node-b        Up 65s (healthy)          8001/tcp
node-c        Up 65s (healthy)          8002/tcp
nginx         Up 65s (healthy)          0.0.0.0:443->443/tcp
opc-server-1  Up 65s                    4840/tcp
opc-server-2  Up 65s                    4841/tcp
opc-server-3  Up 65s                    4842/tcp
\end{verbatim}

API health check responds with 200 OK. SQLite databases created successfully in volumes.
Logs show no critical errors. OPC UA connections established within 30 seconds of startup.
...
\begin{enumerate}
    ...
    \item Query count: \texttt{SELECT COUNT(*) FROM data\_points} on node-a - note N;
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/screenshot-setup.png}
\caption{Setup script execution showing certificate generation and credentials creation}
\label{fig:screenshot-setup}
\end{figure}
...
\subsubsection{Method 1: Swagger UI (Recommended for Beginners)}
...
\subsubsection{Method 2: Command Line (curl)}

For automation and scripting, use curl or similar tools.
...
\begin{description}
    \item[System Architect] Defined the Hybrid Client-Server/P2P architecture and 
    ...
    \item[Backend Engineer] Implemented the three core distributed protocols:
    \begin{itemize}
    ...
    \item[DevOps Engineer] Orchestrated the multi-container deployment using Docker 
    ...
    \item[Quality Assurance] Designed and executed functional tests for
    functional requirements, performance tests and fault tolerance tests including network
    partition simulation with iptables.
    
    \item[Technical Writer] Produced comprehensive documentation covering architectural 
...
The system successfully implements sophisticated distributed systems patterns without 
