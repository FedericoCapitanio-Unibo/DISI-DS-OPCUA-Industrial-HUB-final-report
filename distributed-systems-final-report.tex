...
\begin{itemize}
    \item Latest value: \texttt{docker exec hub-node-a sqlite3 /app/data/hub.db "SELECT * WHERE tag=? AND server\_name=? ORDER BY lc DESC LIMIT 1"}
    \item History: \texttt{docker exec hub-node-a sqlite3 /app/data/hub.db "SELECT * WHERE tag=? AND timestamp BETWEEN ? AND ? ORDER BY lc ASC"}
\end{itemize}
...
\begin{enumerate}
    ...
    \item Query database node-a: \texttt{docker exec hub-node-a sqlite3 /app/data/hub.db "SELECT COUNT(*) FROM data\_points WHERE server\_name='OPCServer1'"};
...
curl -X POST https://localhost/api/v1/admin/opc-servers \
  ...
  -d '{"server_name":"OPCServer4","endpoint":"opc.tcp://opc-server-4:4843"}'
    ...
    \item Query database on node-b and node-c: \texttt{docker exec hub-node-b sqlite3 /app/data/hub.db "SELECT * FROM server\_configs WHERE server\_name='OPCServer4'"; docker exec hub-node-c sqlite3 /app/data/hub.db "SELECT * FROM server\_configs WHERE server\_name='OPCServer4'"};
...
\paragraph{Test Procedure}
\begin{enumerate}
    \item Identify a recent data point on node-a: \texttt{docker exec hub-node-b sqlite3 /app/data/hub.db "SELECT * FROM data\_points ORDER BY lamport\_clock DESC LIMIT 1"};
    \item Note the LC of the data point (es. LC=5234)
    \item Query node-b: \texttt{docker exec hub-node-b sqlite3 /app/data/hub.db "SELECT * FROM data\_points WHERE lamport\_clock=5234"};
...
\begin{enumerate}
    ...
    \item Query count: \texttt{docker exec hub-node-a sqlite3 /app/data/hub.db "SELECT COUNT(*) FROM data\_points"} on node-a - note N;
